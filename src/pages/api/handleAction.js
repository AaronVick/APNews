...
export default async function handleAction(req, res) {
    ...
    try {
        ...
        const imageUrl = `https://place-hold.it/${IMAGE_WIDTH}x${IMAGE_HEIGHT}/4B0082/FFFFFF/png?text=${formattedTitle}&font=arial&size=48&align=center,center`;
        ...
        const errorImageUrl = `https://place-hold.it/${IMAGE_WIDTH}x${IMAGE_HEIGHT}/4B0082/FFFFFF/png?text=${encodeURIComponent('Error: ' + error.message)}&font=arial&size=30&align=center,center`;
        res.status(200).setHeader('Content-Type', 'text/html').send(`
          <!DOCTYPE html>
          <html>
            <head>
